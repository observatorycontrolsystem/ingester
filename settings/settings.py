...
CALIBRATION_TYPES = os.getenv('CALIBRATION_TYPES', "BIAS,DARK,SKYFLAT,EXPERIMENTAL").strip(',').replace(' ','').split(',')

# Proposals including these strings will be considered public data
PUBLIC_PROPOSALS = os.getenv('PUBLIC_PROPOSALS', "EPO,calib,standard,pointing").strip(',').replace(' ','').split(',')
